...
def get_shortestpath_list(start_term_rel_sub, network, index_table_network):
    # 针对每个起点构造终点列表
    all_reslut = []
    for item in start_term_rel_sub:
        result = get_shortestpath(item["start_node"], item["term_node_list"],
                                  network, index_table_network)
        all_reslut += result
    return all_reslut


def getallshortestpath(start_term_rel, network, index_table_network):
    # 多进程求最短路
    import multiprocessing
    from tqdm import tqdm
    lenth_inode_list = len(start_term_rel)
    num_process = 15  # 进程数
    sub_len_inode = lenth_inode_list % num_process
    mp = multiprocessing.Pool(processes=num_process)
    mplist = []
    for i in range(num_process):
        if i != num_process-1:
            sub = start_term_rel[i*sub_len_inode:(i+1)*sub_len_inode]
        else:
            sub = start_term_rel[i*sub_len_inode:lenth_inode_list]
        # 异步（并行）执行get_shortestpath_list函数
        mplist.append(mp.apply_async(get_shortestpath_list,
                                     args=(sub, network,
                                           index_table_network,)))
    mp.close()
    mp.join()
    ...
    for result in tqdm(mplist):
        all_shorest_path += result.get()
    return all_shorest_path
...
def initialization(network, od, index_table, start_term_rel):
    ...
    all_shorest_path = getallshortestpath(start_term_rel, network,
                                          index_table)
...
def FW_algorithm(network, od):
    ...
    while abs(delta) > 0.0001:
        ...
        all_shortest_path = getallshortestpath(start_term_rel, network,
                                               index_table_network)
...
if __name__ == '__main__':
    ...
    network_dict_2 = network_2.to_dict('list')
    od_dict_2 = od_2_new.to_dict('list')
